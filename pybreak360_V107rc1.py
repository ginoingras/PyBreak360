...
try:
	...
	#from src.pybreak360_varglob import *

	#import pybreak360_bullets
	#from src.pybreak360_bullets import *
	#from src.pybreak360_bullets import Bullet
	#print bullets
...
bullet1 = pybreak360_bullets.Bullet((50, 50), (ang, speed), th_E, 0, 1, 1) #((px,py),(angle,speed), threadEmission, idx_bullet, imgbullet, status-porteur)
bullet2 = pybreak360_bullets.Bullet((40, 70), (0.88, 0), th_E, 1, 2, 2)
bullet3 = pybreak360_bullets.Bullet((700, 700), (4.76, 0), th_E, 2, 3, 3)
bullet4 = pybreak360_bullets.Bullet((700, 700), (2.76, 0), th_E, 3, 4, 4)
...
global BatDim, BatDimTime, InvertTime
BatDim = [0, 0, 0, 0]
BatDimTime = [0, 0, 0, 0]
InvertTime = [0, 0, 0, 0]

global pybreak360version
pybreak360version = "1.0.7rc1"
									...
									for idx, bullet in enumerate(bullets): # if speed == 0, missil must follow bat's porteur
										...
										if (bporteur-1) == playerno:
											...
											if bspeed == 0:
												...
												bullets[idx][0].setposxyvect((bx,by), (math.pi+players[bporteur-1][2], ParBulletSpeed), players[bporteur-1][2])
												...
												for nb, player in enumerate(players): # broadcast, informe clients
													if player[0]!="free" and nb != self.no:
														...
														bang = bvector[0]
														bspeed = bvector[1]
														conn.send("T:%s:%s:%s:%s:%s:%s:%s\n" %(idx, bullets[idx][0].getimg(), bx, by, bang, ParBulletSpeed,players[bporteur-1][2]))
						...
						for xligne, ligne in enumerate(value.split()):
							if xligne >= value.count("\n"): # if line not ended, may be the last line
								...
								if ligne[0][0] == 'T': # T:1:image_bullet:x:y:angle:speed (broadcast)
									...
									bullets[idx][0].setposxyvect((newx,newy), (newangle,newz), neworiented)
								...
								if ligne[0][0] == 'A': # conn (maintenu par serveur) ou P1, P2, P3, P4 (broadcast)
									...
									if ligne[1] == "NUCLEARON" :
										idx = eval(ligne[0][1])
										balls[idx][0].setnuclear(time.time() + 10)
									if ligne[1] == "NUCLEAROFF" :
										idx = eval(ligne[0][1])
										balls[idx][0].setnuclear(0)
									if ligne[1] == "BIGBALLON" :
										idx = eval(ligne[0][1])
										balls[idx][0].setbigball(time.time() + 7)
									if ligne[1] == "BIGBALLOFF" :
										idx = eval(ligne[0][1])
										balls[idx][0].setbigball(0)
...
class Bullet(pygame.sprite.Sprite):
	...
	def __init__(self, (xy), vector, th_E, idx, img, porteur):
		...
		self.rayon = +2000 #rayon where is bullet, +1000 to be sur it start from far

		self.rads = 0
		self.vector = vector
		self.hit = 0
 		self.collision = [False] * 9 # 8 points + center
	...
	def update(self):
		...
		if self.vector[1] != 0: #check collid only if bullet mouving and don't follow bat
			...
			if self.th_E == 0:
				...
				if not self.area.contains(newpos):
					...
					#if (rayonbullet) > rayon:
					if (rayonbullet) > rayon - 10: #bat size
						#test we're yet within bat's trajectory
						#if (rayonbullet) > self.rayon: # and not self.hit:
						if (rayonbullet) < rayon + 10: # and not self.hit:
		...
		if self.th_E != 0:
			if has_changed == True: # test not nedeed as in else
				...
				self.th_E.sendMsg("T:%s:%s:%s:%s:%s:%s:%s\n" %(self.idx, self.img, self.rect.x, self.rect.y, angle, z, players[porteur-1][2]))
				# update porteur
				# Warning: double emploi si on a deja touche, but nedeed update bullets
				self.th_E.sendMsg("S%s:%s\n" %(porteur-1, players[porteur-1][3]))
	...
	def setposxyvect(self, (xy), vector, angle):
		...
		if angle != "free":
			...
			self.image, self.rect = rot_center(self.imageOri, self.rectOri, 90 - math.degrees(float(angle)))
		else:
...
def blit_level(levelNumber):
	...
	#Re-blit bricks
	for idx, brick in enumerate(bricks):
		bricks[idx][0].updatefenetre(fenetre)
		try: # thread may lock bilt
...
#################################################	
def designer_blit_level_array(levelNumber, designer_bricks_select, designer_bricks_array, MenuBottom):
	...
	for idx, MenuBottomItem in enumerate(MenuBottom):
		try: # thread may lock bilt
			fenetre.blit(MenuBottom[idx][0], (MenuBottom[idx][1][0],MenuBottom[idx][1][1]) )
		except:
			pass

	#Re-blit bricks select menu
	for idx, brick in enumerate(designer_bricks_select):
		designer_bricks_select[idx][0].updatefenetre(fenetre)
		try: # thread may lock bilt
			designer_bricks_select[idx][1].draw(fenetre)
		except:
			pass
	...
	for idx, brick in enumerate(designer_bricks_array):
		for idx2, brick2 in enumerate(brick):
			designer_bricks_array[idx][idx2][0].updatefenetre(fenetre)
			try: # thread may lock bilt
				designer_bricks_array[idx][idx2][1].draw(fenetre)
			except:
				pass
...
#################################################	
def designer_level_get_array(levelClient, ParShiftBrick, ParBrickSpace):
			...
			if ylevel2 != 0:
				...
				if ParShiftBrick:
					if xlevelPos%2 == 0: #line even
						brick = Brick((fenetre_center[0]-bspacey*linelenght/2+bspacey/4+bspacey*ylevelPos, fenetre_center[1]-bspacex*linelenght+bspacex*xlevelPos), \
						th_E, 0, ylevel2, 1)
					else: #line odd
						brick = Brick((fenetre_center[0]-bspacey*linelenght/2-bspacey/4+bspacey*ylevelPos, fenetre_center[1]-bspacex*linelenght+bspacex*xlevelPos), \
						th_E, 0, ylevel2, 1)
				else: # don't shift brick in level
					brick = Brick((fenetre_center[0]-ParBrickSpace*linelenght/2+ParBrickSpace*ylevelPos, fenetre_center[1]-ParBrickSpace*linelenght/2+ParBrickSpace*xlevelPos), \
					th_E, 0, ylevel2, 1)
...
#################################################	
def designer_level(levelNumber):
	...
	global ParScreenSize, ParBallSpeedSlow, ParBallSpeedFast, ParDwngrdBrick
	global levels, levelName, levelBackGnd, levelClient
	global nblevels
	...
	for idx, bullet in enumerate(bullets):
		...
		# in thread reception
		#th_E.sendMsg("T:%s:%s:%s:%s:%s:%s\n" %(idx, idx+1, 0, 0, 0, 0))
		th_E.sendMsg("T:%s:%s:%s:%s:%s:%s:%s\n" %(idx, idx+1, 0, 0, 0, 0, players[idx][2]))
	...
	levelClient = levels[levelNumber][9:] #bricks list for actual level
	...
	designer_bricks, designer_bricks_array = designer_level_get_array(levelClient, ParShiftBrick, ParBrickSpace)
	...
	while designer_running:
		#menu bottom [(LevelName, LevelNameRect)]
		MenuBottom =[]
		font = pygame.font.Font("freesansbold.ttf", 14)			
		font.set_bold(True)
		#levelname
		text = font.render("%s, right/left click change Parameters, S to save" %(levelName), 1, (0, 0, 0)) #RGBA
		...
		textpos.x = fenetre.get_rect()[0]
		textpos.y = fenetre.get_rect()[2]-(textpos[3]*2)
		...
		textpos.x = fenetre.get_rect()[0]
		textpos.y = fenetre.get_rect()[2]-textpos[3]
		MenuBottom.append((text,textpos))
		#ParBallSpeedFast
		...
		#designer_blit_level(levelNumber, designer_bricks_select, designer_bricks)
		designer_blit_level_array(levelNumber, designer_bricks_select, designer_bricks_array, MenuBottom)
		...
		for event in pygame.event.get():    # Waiting for events
			...
			if (event.type == KEYDOWN):
				...
				if (event.key == K_s): #save levelpack
					...
					#print levelClient
					levels[levelNumber] = []
					levels[levelNumber].append(levelName)# = levels[levelNumber][0]
					levels[levelNumber].append(levelBackGnd)# = levels[levelNumber][1]
					levels[levelNumber].append(levelBackSound)# = levels[levelNumber][2]
					levels[levelNumber].append(str(ParBallSpeedSlow))# = eval(levels[levelNumber][3]) # is level parameter
					levels[levelNumber].append(str(ParBallSpeedFast))# = eval(levels[levelNumber][4]) # is level parameter
					levels[levelNumber].append(str(ParDwngrdBrick))# = eval(levels[levelNumber][5]) # is level parameter
					levels[levelNumber].append(str(ParShiftBrick))# = eval(levels[levelNumber][6]) # is level parameter
					levels[levelNumber].append(str(ParBrickSpace))# = eval(levels[levelNumber][7]) # is level parameter
					levels[levelNumber].append(str(ParScreenSize[0]))# = eval(levels[levelNumber][8]) # is level parameter
					levelClient = [tuple(b) for b in designer_level] #bricks list for actual level
					for lines in levelClient:
						#levels[levelNumber].append(levelClient)# = levels[levelNumber][9:] #bricks list for actual level
						levels[levelNumber].append(lines)# = levels[levelNumber][9:] #bricks list for actual level
		...
		for idx, designerBrick in enumerate(designer_bricks):
			...
			designerBrickRectCenter = designerBrickRect.center			
			if (abs(mouseX - designerBrickRectCenter[0]) < 10) and (abs(mouseY - designerBrickRectCenter[1]) < 10):
				fenetre.blit(BricksColor[23], (designerBrickRect[0],designerBrickRect[1]) )
...
def designer_level_save(levelpack):
	...
	try:
		...
		global nblevels, levels
		for idx, line in enumerate(levels):
			#print (idx)
	...
	for xlevelPos, xlevel in enumerate(levelClient):
		for ylevelPos, ylevel in enumerate(xlevel):
			...
			if ylevel != 0:
				...
				if ParShiftBrick:
					if xlevelPos%2 == 0: #line even
						#brick = Brick((fenetre_center[0]-bspacey*linelenght/2+bspacey*ylevelPos, fenetre_center[1]-bspacex*linelenght+bspacex*xlevelPos), \
						brick = Brick((fenetre_center[0]-bspacey*linelenght/2+bspacey/4+bspacey*ylevelPos, fenetre_center[1]-bspacex*linelenght+bspacex*xlevelPos), \
						th_E, 0, ylevel, 1)
					else: #line odd
...
def main():
	...
	global PenalityDelays   # when lose ball
	global hiscores
	global WallTime, BckGndWall, BckGndWall2
	global BatDim, BatDimTime, InvertTime
	#BatDim = [0, 0, 0, 0]
	#BatDimTime = [0, 0, 0, 0]
	#InvertTime = [0, 0, 0, 0]
	...
	#rand = ((0.1 * (random.randint(5,8))))
	th_E = 0 # server isn't started yet
	bullet1 = Bullet((50, 50), (ang, speed), th_E, 0, 1, 1) #((px,py),(angle,speed), threadEmission, idx_bullet, imgbullet, status-porteur)
	...
	while continuer:
						...
						if (bporteur-1) == playerno: # porteur is local client and so exist, except on deconnection
							...
							if bspeed == 0:
								...
								if (players[idx][4]!=0):
									...
									bullets[idx][0].setposxyvect((bx,by), (bangle, ParBulletSpeed), players[bporteur-1][2])
									#th_E.sendMsg("T:%s:%s:%s:%s:%s:%s\n" %(idx, bullets[idx][0].getimg(), bx, by, bangle, ParBulletSpeed))
									th_E.sendMsg("T:%s:%s:%s:%s:%s:%s:%s\n" %(idx, bullets[idx][0].getimg(), bx, by, bangle, ParBulletSpeed, players[bporteur-1][2]))
		...
		for idx, brick in enumerate(bricks):
			...
			try: # thread may lock bilt
				bricks[idx][1].draw(fenetre)
				bricks[idx][0].showBrickScoreUpdate(fenetre)
			except:
				pass
			...
			if (players[idx][2]!="free"):
				...
				if (players[idx][4]!=0):
					...
					if bspeed == 0:
						...
						bullets[idx][0].setposxyvect((bx,by), (math.pi-players[bporteur-1][2], 0), players[idx][2])
		...
		if (players[playerno][2] != "free"):
			if (players[playerno][5]!=0): # player got balls?
				...
				try: # thread may lock bilt
					...
					pass
				#1.0.7 draw nuclear
				if balls[playerno][0].getnuclear() !=0:
					ballpos = balls[playerno][0].getposxyvect()
					ballpos = (ballpos[0], ballpos[1])
					try: # thread may lock bilt
						if balls[playerno][0].getbigball()==0:
							fenetre.blit(g_ballnuclear, ballpos)
						else:
							fenetre.blit(g_bigballnuclear, ballpos)
					except:
						pass
