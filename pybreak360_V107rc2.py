...
try:
	...
	#from pybreak360_bullets import *
	#print bullets
						...
						for xligne, ligne in enumerate(value.split()):
							if xligne >= value.count("\n"): # if line not ended, may be the last line
								...
								if ligne[0][0] == 'P': # P1, P2, P3, P4 (broadcast)
									...
									load_batpng(nb, varglob.playersbat[nb][5])
									continue
								...
								if ligne[0][0] == 'T': # T:1:image_bullet:x:y:angle:speed (broadcast)
									...
									#varglob.bullets[idx][0].setposxyvect((newx,newy), (newangle,newz))
...
#WARNING: doublon in designer
#################################################
def load_backgnd0(name):
...
def main():
	...
	nbbricks = 0
	#global BckGndWall, BckGndWall2 # from pybreak360_sprites import *
	...
	a="""
	varglob.anouncetexte = ["PyBreak360 V%s" %(varglob.pybreak360version)]
	varglob.anouncetime = 4
	varglob.anouncetimeactu = 0
	varglob.anouncealpha = 180 #128
	...
	while continuer:
		...
		#if (varglob.players[0][0]!="free"):
			howplayers += 1
		if (varglob.players[1][2]!="free"):
		#if (varglob.players[1][0]!="free"):
			howplayers += 1
		if (varglob.players[2][2]!="free"):
		#if (varglob.players[2][0]!="free"):
			howplayers += 1
		if (varglob.players[3][2]!="free"):
		#if (varglob.players[3][0]!="free"):
			howplayers += 1
		...
		if howplayers > 1: #in multiplayers, always have ball, game finish with level (only one level played)
			if nbbricks != 0:
				endgame = False
			a="""
			for idx in xrange(0, 4):
